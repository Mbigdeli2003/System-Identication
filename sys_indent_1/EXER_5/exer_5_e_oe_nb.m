...
for nb=1:1:10;
    i=i+1;
    model_oe =  oe(yk,[nb nc 0]);%m  = bj(data,[nb nc nd nf nk]) A=1
    Yhat = sim(model_oe,[r]);%Simulate dynamic system
...
nb=nmin
model_oe_best =  oe(yk,[nb nc 0])
Yhat = sim(model_oe_best,[r]);%Simulate dynamic system
SSE=0;%Calculating SS
...
legend('real output','OE model output')
grid on
%Specify discrete transfer functions 
discreteTF0=filt(sys.B,sys.F)*filt(1,sys.A);
discreteTFoe=filt(model_oe_best.B,model_oe_best.F)*filt(1,model_oe_best.A);
subplot(2,2,3)
pzplot(discreteTF0,'b')%Plot pole-zero map
legend('System')
subplot(2,2,4)
pzplot(discreteTFoe,'r')%Plot pole-zero map
legend('OE model')