...
phi_2=zeros(15,5);
alpha =1e6;
% P(:,:,13) = alpha*eye(length(phi(1,:)));
P_1(:,:,12) =alpha*eye(length(phi_1(1,:)));
P_2(:,:,12) = alpha*eye(length(phi_2(1,:)));
yk=zeros(1,length(U));

nv_k_hat=repmat((1/alpha)*ones(13,1),1,15);
nv_k_hat_1=(1/alpha)*ones(5,15);
nv_k_hat_2=(1/alpha)*ones(5,15);
N=2;
teta=[A(2:end) B(2:end)]';
...
%    
 ...
 phi_1(k*q,:)=[phi(k*q,1:5)];
 phi_2(k*q,:)=phi(k*q,6:10);
    P_1(:,:,k*q)=P_1(:,:,k*q-q)-((P_1(:,:,k*q-q)*phi_1(k*q,:)'*phi_1(k*q,:)*P_1(:,:,k*q-q))/(1+phi_1(k*q,:)*P_1(:,:,k*q-q)*phi_1(k*q,:)'));
    P_2(:,:,k*q)=P_2(:,:,k*q-q)-((P_2(:,:,k*q-q)*phi_2(k*q,:)'*phi_2(k*q,:)*P_2(:,:,k*q-q))/(1+phi_2(k*q,:)*P_2(:,:,k*q-q)*phi_2(k*q,:)'));


  nv_k_hat_1(:,k*q)=nv_k_hat_1(:,k*q-q)+P_1(:,:,k*q)*phi_1(k*q,:)'*(yk(k*q)-phi(k*q,:)*nv_k_hat(:,k*q-q));
  nv_k_hat_2(:,k*q)=nv_k_hat_2(:,k*q-q)+P_2(:,:,k*q)*phi_2(k*q,:)'*(yk(k*q)-phi(k*q,:)*nv_k_hat(:,k*q-q));
  nv_k_hat_2_c=[nv_k_hat_2(:,k*q)' D(2) D(3) D(4)];
nv_k_hat(:,k*q)=[nv_k_hat_1(:,k*q);nv_k_hat_2_c'];
...
teta_hat_A(:,k)=teta_hat(1:3,k);
teta_hat_B(:,k)=teta_hat(5:7,k);
teta_hat_n(:,k)=[teta_hat_A(:,k);teta_hat_B(:,k)];
delta(k)=norm((teta_hat_n(:,k)-teta),1)/norm(teta,1)/4;
